...
function customAssign(...objects) {
  ...
  const mergetObject = Object.assign(...objects);
  ...
  for (const key in mergetObject) {
    if (typeof mergetObject[key] === "number") {
      mergetObject[key] = String(mergetObject[key]);
    }
  }
  return mergetObject;
}
...
function filterObjectsByKey(arr, key) {
  ...
  const fill = arr.filter((obj) => obj.hasOwnProperty(key));
...
function checkAndPreventExtensions(obj) {
  ...
  const extens = Object.preventExtensions(obj);
  const isExtens = Object.isExtensible(extens);
...
function sealAndCheck(obj) {
  ...
  const isSealed = Object.isSealed(seal);
  return isSealed;
}
...
function getObjectValuesSum(obj) {
  ...
  // const allObject = Object.values(obj);
  // // Обчислюємо суму значень
  // const sum = Array.reduce(allObject);
  // // Повертаємо суму
  // return sum;
}
